...
@NgModule({
    ...
    providers: [
        provideTranslations('node-browser', 'assets/node-browser'),
        provideExtensionConfig(['mgmt.node-browser.icons.json', 'mgmt.node-browser.columns.json', 'mgmt.node-browser.view-items.json']),
    ],
})
export class ContezzaNodeBrowserSharedModule {
    constructor(readonly extensions: ExtensionService) {
